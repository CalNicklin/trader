...
import {
  fetchNews,
  filterNewsForSymbols,
  type NewsItem,
} from "./sources/news-scraper.ts";
import {
  getYahooFundamentals,
  getYahooQuote,
} from "./sources/yahoo-finance.ts";
import {
  addToWatchlist,
  getActiveWatchlist,
  getStaleSymbols,
...
async function discoverNewStocks(): Promise<void> {
  try {
    ...
    log.info(
      { candidates: candidates.length, added },
...
export async function researchSymbol(
  ...
  try {
    ...
    log.debug(
      { symbol },
      "Historical bars not available (IBKR might be disconnected)",
    );
  }
