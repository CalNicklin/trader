...
function bollingerBands(
  ...
  if (middle === null)
    return { upper: null, middle: null, lower: null, percentB: null };
...
function classifyTrend(
  ...
  if (sma200 !== null && price > sma20 && sma20 > sma50 && sma50 > sma200)
    return "strong_up";
  if (sma200 !== null && price < sma20 && sma20 < sma50 && sma50 < sma200)
    return "strong_down";
...
function detectMacdCrossover(
  closes: readonly number[],
): TechnicalIndicators["macdCrossover"] {
...
function classifyVolumeTrend(
  ...
  if (volume5Avg === null || volume20Avg === null || volume20Avg === 0)
    return null;
...
export function computeIndicators(
  ...
  return {
    ...
    priceVsSma20Pct: sma20Val
      ? ((currentPrice - sma20Val) / sma20Val) * 100
      : null,
    priceVsSma50Pct: sma50Val
      ? ((currentPrice - sma50Val) / sma50Val) * 100
      : null,
    rsi14: rsi14Val,
    ...
    distanceFromHigh52w:
      high52w > 0 ? ((high52w - currentPrice) / high52w) * 100 : null,
    distanceFromLow52w:
      low52w > 0 ? ((currentPrice - low52w) / low52w) * 100 : null,
  };
}

const barsCache = new Map<
  string,
  { bars: HistoricalBar[]; fetchedAt: number }
>();
...
export function formatIndicatorSummary(ind: TechnicalIndicators): string {
  ...
  if (ind.priceVsSma50Pct !== null) {
    parts.push(
      `Price vs SMA50: ${
        ind.priceVsSma50Pct > 0 ? "+" : ""
      }${ind.priceVsSma50Pct.toFixed(1)}%`,
    );
  }
  if (ind.rsi14 !== null) {
